...
function generate_morph {
	...
	${BIN_DIR}/vabamorf-palakuju *.ok *.1sok *.1pok || exit 1
	...
	${BIN_DIR}/vabamorf-conv11 *.ok-p *.sok-p || exit 1
	...
	${BIN_DIR}/vabamorf-conv4 -daeesti.ini -psuf.ini -jaeesti.jrk|| exit 1
	...
	${BIN_DIR}/vabamorf-init3 || exit 1
	cp aeesti1.dct et.dct || exit 1

	cp et.dct ${BIN_DIR}/et.dct

	echo "== Morfi sõnastik failis:" ${BIN_DIR}/et.dct
}
